...
#define inc_counter_np(counter) \
  ...
  address generate_call_stub(address& return_address) {
    ...
    // stub code
    __ enter(); // 保存调用现场
    __ movptr(rcx, parameter_size);              // parameter counter 计算java函数参数个数
    __ shlptr(rcx, Interpreter::logStackElementSize); // convert parameter count to bytes 把参数个数转换为字节数
    __ addptr(rcx, locals_count_in_bytes);       // reserve space for register saves 保存几个下面几个寄存器的值
    __ subptr(rsp, rcx); // 动态分配堆栈内存
